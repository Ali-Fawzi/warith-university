    ...
    return (
        <Link to={`/courses/${course.id}`} className='shadow-lg rounded-xl bg-white flex flex-col items-center justify-center'>
            <img
                ...
                className='object-cover overflow-hidden w-[400px] h-[320px] rounded-xl'
                loading='lazy'
            />
            <div className='flex flex-col gap-2 w-full p-4'>
                <p className='text-brand text-right'>{course.title}</p>
                <p dir={'RTL'} className='text-xl text-right font-bold min-h-14 truncate'>{course.description}</p>
